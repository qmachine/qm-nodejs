...
(function () {
 ...
 // Pragmas
    ...
    /*properties
        ...
        get_avar, get_list, isMaster, join, key, '$key', length, log, map, now,
        '$now', persistent_storage, run, set_avar, shift, slice, sqlite, split,
        '$status', stringify
    */
         ...
         // This function needs documentation.
            ...
            db.get(query, args, function (err, row) {
                ...
                if (err !== null) {
                    if (err.errno === 5) {
                        if (typeof setImmediate === 'function') {
                            setImmediate(get_avar, params, callback);
                        } else {
                            setTimeout(get_avar, 0, params, callback);
                        }
                        return;
                    }
                    callback(err, row);
             ...
             // The row exists, so update its expiration date.
                ...
                db.get(query, args, function f(err) {
                 // This function needs documentation.
                    if (err !== null) {
                        if (err.errno === 5) {
                            if (typeof setImmediate === 'function') {
                                setImmediate(db.get, query, args, f);
                            } else {
                                setTimeout(db.get, 0, query, args, f);
                            }
                            return;
                        }
            ...
            db.all(query, args, function (err, rows) {
             // This function needs documentation.
                if (err !== null) {
                    if (err.errno === 5) {
                        if (typeof setImmediate === 'function') {
                            setImmediate(get_list, params, callback);
                        } else {
                            setTimeout(get_list, 0, params, callback);
                        }
                        return;
                    }
                }
                var x = (rows instanceof Array) ? rows : [];
                callback(err, JSON.stringify(x.map(function (row) {
                 // This function needs documentation.
                    return row.key;
                })));
                return;
