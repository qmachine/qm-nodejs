...
(function () {
 ...
 // Pragmas
    ...
    /*properties
        api, apply, avar_ttl, box, buffer, cmd, collect_garbage,
        'content-length', 'Content-Type', create, createServer, Date,
        ...
        length, listen, log, match, match_hostname, max_body_size,
        max_http_sockets, maxSockets, method, mime_type, on, parse, pattern,
        persistent_storage, pid, port, push, replace, resolve, set_avar, slice,
        sort, splice, split, sqlite, static_content, status, stringify, test,
        timestamp, toString, toUTCString, trafficlog_storage, unroll, url,
        warn, worker_procs, writeHead
    */

 // Declarations

    var cluster, collect_garbage, configure, corser, http, // is_Function,
        katamari, path, spawn_workers, warn;

 // Definitions

    cluster = require('cluster');

    collect_garbage = function (f, settings) {
     // This function needs documentation.
        if (cluster.isWorker) {
            return;
        }
     /*
        if (settings.hasOwnProperty('gc_interval') === false) {
            throw new Error('WTF');
        }
     */
        setInterval(f, settings.gc_interval * 1000);
        return;
    };
 ...
 */
 ...
 // Out-of-scope definitions
    ...
    exports.launch = function (obj) {
     // This function needs documentation.
        /*jslint unparam: true */
        var config, corsify, defs, hang_up, log, rules, save, server,
            static_content;
        ...
        if (config.enable_cors === true) {
            ...
            server = http.createServer(function (request, response) {
             // This function needs documentation.
                ...
                corsify(request, response, function () {
                 // This function needs documentation.
         ...
         */
            ...
            collect_garbage(defs.collect_garbage, config);
        }
        if (config.enable_web_server === true) {
            config.static_content = path.resolve(config.static_content);
