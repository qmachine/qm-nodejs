...
(function () {
 ...
 // Pragmas
    ...
    /*properties
        api, apply, avar_ttl, box, buffer, cf_ipcountry, 'cf-ipcountry', cmd,
        collect_garbage, connection, 'Content-Type', 'content-length', create,
        createServer, Date, enable_api_server, enable_CORS, enable_www_server,
        end, error, fork, gc_interval, get_box_key, get_box_status,
        globalAgent, handler, hasOwnProperty, headers, host, hostname,
        'if-modified-since', ip, isMaster, isWorker, join, key, last_mod_date,
        'Last-Modified', last_modified, launch, length, listen, log, match,
        match_hostname, max_fu_size, max_http_sockets, maxSockets,
        max_upload_size, method, mime_type, on, parse, pattern,
 ...
 // Out-of-scope definitions
    ...
    exports.launch = function (obj) {
     // This function needs documentation.
        /*jslint unparam: true */
        var config, defs, enable_cors, hang_up, log, rules, save, server,
            static_content;
        ...
        log = function (request) {
         // This function needs documentation.
            ...
            if (request.headers.hasOwnProperty('cf-ipcountry')) {
             ...
             // storing the header with a hyphen inside can cause problems in
             // query languages like Hive. Thus, we use a hyphen convention.
        ...
        if (config.enable_api_server === true) {
         ...
         // These are mainly here for debugging at the moment ...
            ...
            rules.push({
                method:  'GET',
                pattern: /^\/(?:box|v1)\/([\w\-]+)\?status=([A-z0-9]+)$/,
                handler: function (request, response, params) {
                 // This function needs documentation.
                    log(request);
                    var callback;
